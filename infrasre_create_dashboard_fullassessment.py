...
def get_assessment_iops(allocated_iops, cloudwatch_data, iops_datapoints, iops_expression_match, volume_id, start_time, end_time, thresholds):
    metric_queries = [
            {
                'Id': 'read_ops',
                'MetricStat': {
    ...
    for i in range(len(thresholds)):
        ...
        for data in zip(response['MetricDataResults'][0]['Timestamps'],
        ...
        if thresholds[i] == 0:
            iops_expression_match_percentage = ((iops_expression_match / iops_datapoints)*100)
            print(f"{vol_name_tag} Total datapoints: {iops_datapoints}. There are {iops_expression_match} datapoint(s) greater than or equal to its allocated IOPS {allocated_iops}. This is {iops_expression_match_percentage} percent in the last {day_range} days.")
            print(f"Recommendations:")
        else:
            if allocated_iops + thresholds[i] >= 16000:
               print(f"{vol_name_tag}: Adding {thresholds[i]} will reach the maximum recommended size for IOPS at 16000")
            else:
                iops_expression_matches_percentage[i] = (iops_expression_matches[i] / iops_datapoints) * 100
                print(f"{vol_name_tag}: Adding {thresholds[i]} IOPS will change the percentage to {iops_expression_matches_percentage[i]:.2f}%.")

            if allocated_iops - thresholds[i] <= 3000:
                print(f"{vol_name_tag}: Decreasing {thresholds[i]} will reach the minimum recommended size for IOPS at 3000")
            else:
                iops_expression_matches_saving_percentage[i]  = (iops_expression_matches_saving[i] / iops_datapoints) * 100
                print(f"{vol_name_tag}: Decreasing {thresholds[i]} IOPS will change the percentage to {iops_expression_matches_saving_percentage[i]:.2f}%.")
...
def create_iops_widget(volume_id, target_region, vol_name_tag, volume_type, allocated_iops):
    widget = {
            "type": "metric",
            "x": 0,
            "y": 0,
...
def create_throughput_widget(volume_id, target_region, vol_name_tag, volume_type, allocated_throughput):
    widget = {
            "type": "metric",
            "x": 0,
            "y": 0,
...
def get_assessment_throughput(allocated_throughput, cloudwatch_data, throughput_datapoints, throughput_expression_match, volume_id, start_time, end_time, thresholds_throughput):
    metric_queries = [
            {
                'Id': 'read_bytes',
                'MetricStat': {
    ...
    for i in range(len(thresholds_throughput)):
        ...
        for data in zip(response['MetricDataResults'][0]['Timestamps'],
            ...
            if thresholds_throughput[i] == 0:
                ...
                projected_threshold[i] = (allocated_throughput * 1000000) + (thresholds_throughput[i] * 1000000)
                if expression_value >= projected_threshold[i]:
                    throughput_expression_matches[i] += 1
                savings_projected_threshold[i] = (allocated_throughput * 1000000) - (thresholds_throughput[i] * 1000000)
        ...
        if thresholds_throughput[i] == 0:
            throughput_expression_match_percentage = ((throughput_expression_match / throughput_datapoints)*100)
            print(f"{vol_name_tag} Total datapoints: {throughput_datapoints}. There are {throughput_expression_match} datapoint(s) greater than or equal to its allocated throughput {allocated_throughput}. This is {throughput_expression_match_percentage} percent in the last {day_range} days.")
            print(f"Recommendations:")
        else:
            if allocated_throughput + thresholds_throughput[i] >= 1000:
                print(f"{vol_name_tag}: Adding {thresholds_throughput[i]} will reach the maximum recommended size for Throughput at 1000")
            else:
                throughput_expression_matches_percentage[i] = (throughput_expression_matches[i] / throughput_datapoints) * 100
                print(f"{vol_name_tag}: Adding {thresholds_throughput[i]} throughput will change the percentage to {throughput_expression_matches_percentage[i]:.2f}%.")

            if allocated_throughput - thresholds_throughput[i] <= 125:
               print(f"{vol_name_tag}: Decreasing {thresholds_throughput[i]} will reach the minimum recommended size for Throughput at 125")
            else:
                savings_throughput_expression_matches_percentage[i] = (savings_throughput_expression_matches[i] / throughput_datapoints) * 100
                print(f"{vol_name_tag}: Decreasing {thresholds_throughput[i]} throughput will change the percentage to {savings_throughput_expression_matches_percentage[i]:.2f}%.")
...
if instance_id:
    ...
    for volume_id in attached_volumes:
        ...
        widgets.append(create_iops_widget(volume_id, target_region, vol_name_tag, volume_type, allocated_iops))
        widgets.append(create_throughput_widget(volume_id, target_region, vol_name_tag, volume_type, allocated_throughput))

        thresholds = [0, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 5000, 9000]
        if allocated_iops != "N/A":
            get_assessment_iops(allocated_iops, cloudwatch_data, iops_datapoints, iops_expression_match, volume_id, start_time, end_time, thresholds)

        thresholds_throughput = [0, 50, 100, 150, 200, 250, 300, 350, 400]
        if allocated_throughput != "N/A":
            get_assessment_throughput(allocated_throughput, cloudwatch_data, throughput_datapoints, throughput_expression_match, volume_id, start_time, end_time, thresholds_throughput)
